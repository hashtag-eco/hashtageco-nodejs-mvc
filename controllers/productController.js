...
const getScrapList = async(uid) => { //인자 uid는 세션의 멤버 아이디
  try { //세션의 멤버id
...
exports.deleteProductScrap = async(req, res) => {
  ...
  try { 
    ...
    if(req.body.category == 'zerowaste') {
      await ZproductScrap.destroy({ //member_id와 product_id를 조건으로 destroy합니다.
        where : {
          member_id : uid,
          product_id : parseInt(req.body.scrapItemId)
        }
    ...
    //삭제할 아이템이 저탄소 상품인 경우
      ...
      await LproductScrap.destroy({ //member_id와 product_id를 조건으로 destroy합니다.
        where : {
          member_id : uid,
          product_id : parseInt(req.body.scrapItemId)
        }
    ...
    //삭제할 아이템이 업사이클링 상품인 경우
      ...
      await UproductScrap.destroy({ //member_id와 product_id를 조건으로 destroy합니다.
        where : {
          member_id : uid,
          product_id : parseInt(req.body.scrapItemId)
        }
...
exports.showProductScrap = async (req, res) => {
  ...
  const scrapList = await ZProduct.findAll({
    attributes: ['product_id', 'product_name', 'price', 'image_link', 'mall_name', 'category', 'category1', 'category2'], //구하고자 하는 칼럼
    //include통해 Member테이블과 연결
    include: {
      model: Member, //model로 연결할 테이블 정의합니다.
      where: { member_id : uid }, //멤버아이디가 세션아이디인 row만 선택합니다.
